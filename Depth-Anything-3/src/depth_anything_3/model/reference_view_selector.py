...
def _finite_l2_normalize(x: torch.Tensor, eps: float = 1e-6) -> torch.Tensor:
    """
    Normalize features in float32 while guarding against NaN/Inf and zero norms.

    This keeps the reference-view scoring path numerically stable even when the
    surrounding forward pass runs under autocast.
    """
...
def select_reference_view(
    ...
    # Feature-based strategies require normalized class tokens. Do this in
    # float32 with finite-value guards to avoid unstable low-precision kernels.
    class_feat = x[:, :, 0]
    img_class_feat = _finite_l2_normalize(class_feat)  # B S C
    ...
    if strategy == "saddle_balanced":
        ...
        feat_norm = torch.nan_to_num(class_feat.float(), nan=0.0, posinf=0.0, neginf=0.0).norm(dim=-1)  # B S

...
class DepthCrafterPipeline(StableVideoDiffusionPipeline):
    ...
    def __call__(
        ...
        # 不再一次性编码全部视频，改为在循环中lazy encoding
        # 这样可以大幅降低显存占用

        # 4. Prepare noise for the video
        noise = randn_tensor(
            video.shape, generator=generator, device=device, dtype=video.dtype
        )
        video_noisy = video + noise_aug_strength * noise  # in [t, c, h, w]
        ...
        # Store original video for later encoding
        video_for_encoding = video
        video_noisy_for_encoding = video_noisy
        ...
        added_time_ids = self._get_add_time_ids(
            ...
            127,
            noise_aug_strength,
            video.dtype,  # 使用video.dtype而不是video_embeddings.dtype
            batch_size,
            1,
            False,
        )  # [1 or 2, 3]
        ...
        while idx_start < num_frames - overlap:
            ...
            print(f"  Processing window [{idx_start}:{idx_end}] ({idx_end - idx_start} frames)")

            # Lazy encoding: 只编码当前window
            if track_time:
                window_encode_start = torch.cuda.Event(enable_timing=True)
                window_encode_end = torch.cuda.Event(enable_timing=True)
                window_encode_start.record()

            video_window = video_for_encoding[idx_start:idx_end]
            video_embeddings_current = self.encode_video(
                video_window, chunk_size=decode_chunk_size
            ).unsqueeze(0)  # [1, window_size, 1024]

            video_noisy_window = video_noisy_for_encoding[idx_start:idx_end]
            video_latents_current = self.encode_vae_video(
                video_noisy_window.to(self.vae.dtype),
                chunk_size=decode_chunk_size,
            ).unsqueeze(0)  # [1, window_size, c, h, w]

            if track_time:
                window_encode_end.record()
                torch.cuda.synchronize()
                encode_time_window = window_encode_start.elapsed_time(window_encode_end)
                encode_time_total += encode_time_window
                print(f"    Encoding time: {encode_time_window:.1f} ms")
            ...
            # 清理当前window的中间变量，释放显存
            del video_window, video_noisy_window, video_embeddings_current, video_latents_current, latents
            torch.cuda.empty_cache()

            idx_start += stride

        # Cast VAE back to fp16 if needed
        if needs_upcasting:
        ...
        if track_time:
            ...
            torch.cuda.synchronize()
            elapsed_time_ms = start_event.elapsed_time(denoise_event)
            print(f"Total encoding time: {encode_time_total:.1f} ms")
            print(f"Total time (encoding + denoising): {elapsed_time_ms} ms")
